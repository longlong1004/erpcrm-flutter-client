...
/// 其它订单导入信息屏幕
class OtherOrderImportScreen extends ConsumerStatefulWidget {
  const OtherOrderImportScreen({super.key});

  @override
  ConsumerState<OtherOrderImportScreen> createState() => _OtherOrderImportScreenState();
}

class _OtherOrderImportScreenState extends ConsumerState<OtherOrderImportScreen> {
  ...
  void _resetFilters() {
    ...
    ref.read(ordersProvider.notifier).fetchOrders(params: {'orderType': 'other', 'imported': true});
    setState(() {
      _currentPage = 0;
    });
  }
  ...
  void _printCertificate(int orderId) {
    // 实现打印合格证功能
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('打印合格证功能已触发，订单ID: $orderId')),
    );
  }
      ...
      children: [
        ...
        Expanded(
          child: RefreshIndicator(
            onRefresh: () => ref.read(ordersProvider.notifier).fetchOrders(params: {'orderType': 'other', 'imported': true}),
            child: ordersAsync.when(
              ...
              error: (error, stack) => Center(
                child: Column(
                  ...
                  children: [
                    Text('加载失败: $error'),
                    ElevatedButton(
                      onPressed: () => ref.read(ordersProvider.notifier).fetchOrders(params: {'orderType': 'other', 'imported': true}),
                      child: const Text('重试'),
                    ),
                  ],
                ),
              ...
              data: (orders) {
                ...
                final tableOrders = otherOrders.map((order) => {
                  ...
                  'operationTime': order.formattedUpdatedAt,
                  'importTime': order.formattedCreatedAt,
                }).toList();
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: DataTable(
                            ...
                            sortColumnIndex: _sortColumn == 'orderNumber' ? 2 :
                                          _sortColumn == 'importTime' ? 21 : null,
                            ...
                            columns: [
                              ...
                              const DataColumn(label: Text('国铁金额'), numeric: true),
                              const DataColumn(label: Text('操作时间')),
                              DataColumn(
                                label: const Text('导入时间'),
                                onSort: (columnIndex, ascending) {
                                  _sort((order) => order['importTime'] as String, 'importTime');
                                },
                              ),
                            ...
                            rows: paginatedOrders.map((order) => DataRow(cells: [
                              ...
                              DataCell(Text(order['operationTime'] as String)),
                              DataCell(Text(order['importTime'] as String)),
                              DataCell(Row(
                                children: [
                                  TextButton.icon(
                                  ...
                                  TextButton.icon(
                                    onPressed: () => _printCertificate(order['id'] as int),
                                    icon: const Icon(Icons.print, size: 16),
                                    label: const Text('打印合格证'),
                                    style: TextButton.styleFrom(
                                      padding: const EdgeInsets.symmetric(horizontal: 8),
                                    ),
                                  ),
