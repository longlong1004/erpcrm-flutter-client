...
import 'package:erpcrm_client/utils/logger_service.dart';
...
class _SidebarState extends ConsumerState<Sidebar> {
  ...
  Widget build(BuildContext context) {
    return Consumer(builder: (context, ref, child) {
      ...
      // 根据屏幕宽度设置侧边栏宽度，确保它不会太宽
      final sidebarWidth = screenWidth > 1200 ? 280.0 : 
                           screenWidth > 768 ? screenWidth * 0.25 : 
                           screenWidth * 0.35; // 减少小屏幕上的宽度比例
      
      // 构建菜单列表，找到薪酬管理菜单项
      final menuItems = _buildMenuItems();
      _salaryMenuItem ??= menuItems.firstWhere((item) => item.title == '薪酬管理', orElse: () => const MenuItem(title: '薪酬管理', icon: Icons.payments_outlined));

      return Container(
        width: sidebarWidth,
        constraints: const BoxConstraints(maxWidth: 320), // 添加最大宽度限制
          ...
          children: [
            ...
            Container(
              ...
              decoration: const BoxDecoration(
                border: Border(
                  bottom: BorderSide(color: Color(0xFF004488), width: 1),
                ),
              ),
              child: Column(
                children: [
                  Icon(
                    Icons.business,
                    size: screenWidth > 768 ? 48.0 : 36.0,
                    color: Colors.white,
                  ),
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: screenWidth > 768 ? 18.0 : 14.0,
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (item.title == '薪酬管理') {
                    return Column(
                      children: [
                        _buildSalaryMenuItem(context, item),
            ...
            if (isAuthenticated)
              Padding(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text('退出登录', style: TextStyle(color: Color(0xFF003366))),
  ...
  // 构建薪酬管理菜单项，恢复默认的下拉展开功能
  Widget _buildSalaryMenuItem(BuildContext context, MenuItem item) {
    return _MenuItemWidget(item: item);
  }
...
// 菜单项组件
class _MenuItemWidget extends ConsumerStatefulWidget {
  final MenuItem item;

  const _MenuItemWidget({required this.item});

  @override
  ConsumerState<_MenuItemWidget> createState() => _MenuItemWidgetState();
}

class _MenuItemWidgetState extends ConsumerState<_MenuItemWidget> {
        ...
        children: [
          ...
          if (_isExpanded)
            Container(
              ...
              decoration: const BoxDecoration(
                border: Border(left: BorderSide(color: Color(0xFF004488), width: 2)),
              ),
              child: Column(
                children: widget.item.children!.map((child) {
                  return Padding(
                    padding: const EdgeInsets.only(top: 4, bottom: 4),
                    child: _MenuItemWidget(item: child),
                  );
                }).toList(),
              ),
            ),
          ...
          return ListTile(
            ...
            onTap: () {
              if (widget.item.route != null) {
                  LoggerService.debug('Navigating to: ${widget.item.route}');
                  
                  // 直接使用完整路由，不再提取主路由
                  final fullRoute = widget.item.route!;
                  
                  // 检查是否已经存在相同完整路由的标签页
                  final tabs = ref.read(tabProvider);
                  final existingTabIndex = tabs.indexWhere((tab) {
                    return tab.route == fullRoute;
                  });
                  
                  if (existingTabIndex != -1) {
                    // 如果存在相同完整路由的标签页，直接激活它
                    ref.read(tabProvider.notifier).setActiveTab(existingTabIndex);
                    // 导航到完整路由，确保显示正确的子页面
                    context.go(fullRoute);
                  } else {
                    // 检查是否已经存在相同主路由的标签页
                    final uri = Uri.parse(fullRoute);
                    final mainRoute = uri.pathSegments.isNotEmpty ? '/${uri.pathSegments.first}' : fullRoute;
                    final existingMainRouteTabIndex = tabs.indexWhere((tab) {
                      final tabUri = Uri.parse(tab.route);
                      final tabMainRoute = tabUri.pathSegments.isNotEmpty ? '/${tabUri.pathSegments.first}' : tab.route;
                      return tabMainRoute == mainRoute;
                    });
                    
                    if (existingMainRouteTabIndex != -1) {
                      // 如果存在相同主路由的标签页，激活它并更新标题和路由
                      ref.read(tabProvider.notifier).setActiveTab(existingMainRouteTabIndex);
                      ref.read(tabProvider.notifier).updateActiveTabTitle(widget.item.title!);
                      ref.read(tabProvider.notifier).updateActiveTabRoute(fullRoute);
                      context.go(fullRoute);
                    } else {
                      // 如果不存在，添加新标签页
